...
| Script                                 | Description |
| -----------                            | ----------- |
0_funs.R       | Setup and specify functions used in subsequent analyses |
1_setup and load KC data.R | Load and tidy data from semi-structured surveys in Kansas City, MO ("KC").  The primary output of this script are `data/derived/structured_surveys_schedule.csv` and `data/derived/structured_surveys_bats_discovered.csv`. |
2_plot and summarize KC records.R | Plots and analyses from semi-structured surveys. |
3_model KC record timing.R | Models fit predict discoveries by day of year from KC surveys. |
4_map KC records.R | Map records from KC surveys |
5_get iNat records.R | Download candidate records from iNaturalist. After manual non-scripted cleaning and stripping of unlicensed data, the output of this script is `data/derived/iNaturalist records.csv`. |
6_analyze iNat records.R | Analysis of records from iNaturalist. |


## data/derived/

Tidied data created or synthesized in the course of this study
| File                                   | Description |
| -----------                            | ----------- |
