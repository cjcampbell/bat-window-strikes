...
API access, environmental rasters, radar data, and manual review steps not included
in this repository, so they are provided as a **provenance record of how the shared
data were produced and are not runnable from a fresh checkout**. Their shareable
outputs are the files in `data/derived/`. Paths to the large external inputs are
left as `<placeholder>` values to be set locally; sources are listed below.

**External data sources** (not redistributed here):
- Nighttime radiance (ALAN): VIIRS VNL v2 annual composite — https://eogdata.mines.edu/nighttime_light/annual/ (Elvidge et al. 2021).
- Building height: global 150-m GEDI-based product (GBH2020; Scientific Data, 2024).
- Land cover: NLCD 2021 — https://www.mrlc.gov/data.
- Migration traffic: nightly NEXRAD radar (Dark Ecology / `data/darkecology/`).
- Administrative boundaries: GADM via the `geodata` package.
...
| `iNaturalist records.csv` | Retained iNaturalist records from data synthesis. Observations associated with restrictive licenses (e.g., "All rights reserved") are shared as observation ID and URL only, alongside our annotations (`notes`) and added georeference/taxonomic fields (`family_name`, `adm0_a3`, `continent`); the observer-supplied fields (description, common/scientific name, username, date) are withheld for those records. Contains columns: <br> `id` = iNaturalist ID number of observation<br> `url` = URL of observation <br> `notes` = Notes derived from manual checks of all observations. <br> `scientific_name` = Scientific name of organism from observation. <br> `common_name` = Common name of organism from observation<br> `description` = Text description provided by observer of observation. <br>  `user_login` = Username of user who made observation <br> `observed_on` = Datetime of observation <br> `license` = License of observation (at time of download)<br> `family_name` = Scientific name of family <br> `adm0_a3` = Three-letter country code where record was made <br>  `continent` = Continent of record |
